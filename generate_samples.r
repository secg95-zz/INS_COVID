...
for( i in 1:10){
    ...
    I_bootstrapped = bootstrap_sample(simulados, length(model$beta), 3)
    bootstraped_model = fit(I_bootstrapped[1:(length(I_bootstrapped)-1)], beta0=beta0, beta_min=beta_min, beta_max=beta_max,
                lambda0=1/6, lambda_min=1/8, lambda_max=1/4, alpha=6.4e+13)
    list.append(bootstraped_betas, i=bootstraped_model$beta)
    #print(bootstraped_betas)
