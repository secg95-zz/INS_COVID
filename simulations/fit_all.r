...
fit_all = function(simulation, name, ignore_beta_diff=NULL, lambda=2^10) {
  ...
  plot(simulation$R, type="l", ylab="R", xlab="t", ylim=c(0, max(c(bayesian_R, ss_R, nicolas_R, nicolas_R2, simulation$R), na.rm=TRUE)))
  lines(bayesian_R, col="blue")
  lines(ss_R, col="green")
  lines(nicolas_R, col="red")
  lines(nicolas_R2, col="purple")
  ...
  rmse = list(
    ...
    "general"=mean((nicolas_R - simulation$R) ^ 2, na.rm=TRUE),
    "general+"=mean((nicolas_R2 - simulation$R) ^ 2, na.rm=TRUE)
  )
