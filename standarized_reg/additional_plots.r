...
p_sick_days = diff(pexp(0:steps, model$lambda, lower.tail=TRUE))
R = NULL
for (t in 1:steps) {
  p_sick_over_s_days = rev(cumsum(rev(p_sick_days[1:t]))) / pexp(t, model$lambda, lower.tail=TRUE)
  R = c(R, sum(p_sick_over_s_days * rev(model$beta[1:t])))
}
